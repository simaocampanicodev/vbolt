      ...
      <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
            ...
            <div className="mb-2 w-full flex-1 min-w-0">
              ...
              <div className="flex flex-col w-full md:max-w-xs mb-2">
                ...
                <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-rose-500 transition-all duration-500" style={{ width: `${xpPercent}%` }}></div>
                </div>
              ...
              {!isOwnProfile && (
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  {currentUser.friends?.includes(profileUser.id) ? (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-rose-500/20 border border-rose-500/30 text-rose-400 text-sm font-medium">
                      <CheckCircle className="w-4 h-4" />
                      Friends
                    </span>
                  ) : currentUser.friendRequests?.some(r => r.fromId === profileUser.id) ? (
                    <div className="flex items-center gap-2">
                      <Button size="sm" onClick={() => setShowAcceptRequestModal(true)} className="bg-rose-600 hover:bg-rose-500">
                        Accept Request
                      </Button>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card noPadding className={`p-4 flex flex-col items-center justify-center ${themeMode === 'dark' ? 'bg-black/40' : 'bg-white/40'}`}>
            ...
            <span className={`text-2xl font-display font-bold ${Number(winrate) > 50 ? 'text-rose-400' : (themeMode === 'dark' ? 'text-zinc-200' : 'text-zinc-900')}`}>{winrate}%</span>
          </Card>
          <Card noPadding className={`p-4 flex flex-col items-center justify-center ${themeMode === 'dark' ? 'bg-black/40' : 'bg-white/40'}`}>
                    ...
                    <div key={stat.map} className="relative h-24 rounded-xl overflow-hidden group border border-white/5">
                      ...
                      <div className="absolute bottom-0 left-0 p-3 w-full">
                        <div className="flex justify-between items-end">
                          ...
                          <div className={`text-lg font-bold font-mono ${stat.winrate >= 50 ? 'text-rose-400' : 'text-zinc-400'}`}>
