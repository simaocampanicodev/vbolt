...
const Layout: React.FC<LayoutProps> = ({ currentView, setCurrentView, children }) => {
    ...
    React.useEffect(() => {
        ...
        const update = () => {
            const diff = Date.now() - queueJoinedAt;
        ...
        <div className={`min-h-screen flex flex-col transition-colors duration-500 ${themeMode === 'dark' ? 'bg-[#050505] text-zinc-200' : 'bg-zinc-50 text-zinc-800'}`}>
            ...
            <header className={`h-20 border-b flex items-center justify-between px-4 lg:px-8 sticky top-0 z-50 backdrop-blur-md ${themeMode === 'dark' ? 'bg-black/60 border-white/5' : 'bg-white/60 border-black/5'}`}>
                ...
                <div className="flex items-center space-x-4 lg:space-x-8">
                    ...
                    <div className="flex items-center cursor-pointer group" onClick={() => isAuthenticated && setCurrentView('home')}>
            ...
            <main className="flex-1 relative w-full max-w-7xl mx-auto p-4 lg:py-8 lg:px-12">
                ...
                <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
                    {themeMode === 'dark' && (
                        <>
                            <div className="absolute inset-0 bg-gradient-to-br from-[#050000] via-[#080000] to-[#0c0000]"></div>
                            <div className="absolute top-1/4 -right-1/4 w-[600px] h-[600px] bg-rose-700/10 blur-[130px] rounded-full mix-blend-screen animate-[pulse_8s_ease-in-out_infinite]"></div>
                            <div className="absolute bottom-1/4 -left-1/4 w-[500px] h-[500px] bg-red-800/15 blur-[140px] rounded-full mix-blend-screen animate-[pulse_12s_ease-in-out_infinite]"></div>
                            ...
                            <div className="absolute inset-0 opacity-[0.05]" style={{ backgroundImage: 'radial-gradient(circle at center, #ffffff 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
                        </>
                    )}
                </div>
